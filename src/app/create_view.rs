...
impl App {
    ...
    fn render_create_form(&mut self, ui: &mut Ui) {
        ...
        Grid::new("create_task_date_grid")
            ...
            .show(ui, |ui| {
                ...
                ComboBox::from_id_salt("create_status_combo")
                    ...
                    .show_ui(ui, |ui| {
                        ui.selectable_value(&mut self.temp_task.status, TaskStatus::Pending, RichText::new(status_items[0]));
                        ui.selectable_value(&mut self.temp_task.status, TaskStatus::WorkInProgress, RichText::new(status_items[1]));
                        ui.selectable_value(&mut self.temp_task.status, TaskStatus::Complete, RichText::new(status_items[2]));
                    });
                ...
                ComboBox::from_id_salt("create_priority_combo")
                    .selected_text(RichText::new(priority_items[self.temp_task.priority as usize]).color(priority_color))
                    .show_ui(ui, |ui| {
                        ui.selectable_value(&mut self.temp_task.priority, Priority::Low, RichText::new(priority_items[0]).color(Color32::GREEN));
                        ui.selectable_value(&mut self.temp_task.priority, Priority::Medium, RichText::new(priority_items[1]).color(Color32::YELLOW));
                        ui.selectable_value(&mut self.temp_task.priority, Priority::High, RichText::new(priority_items[2]).color(Color32::RED));
                    });
                ...
                // Timeline Pickers
                ui.label("Start Date:");
                ui.add(egui_extras::DatePickerButton::new(&mut self.temp_task.start_date).id_salt("create_start_date"));
                
                ui.label("Due Date:");
                ui.add(egui_extras::DatePickerButton::new(&mut self.temp_task.due_date).id_salt("create_due_date"));
        ...
        Grid::new("create_task_text_grid")
            ...
            .show(ui, |ui| {
                ui.label("Project:");
                ui.add(TextEdit::singleline(&mut self.temp_task.project).desired_width(ui.available_width()));
                ui.end_row();

                ui.label("Title:");
                ui.add(TextEdit::singleline(&mut self.temp_task.title).desired_width(ui.available_width()));
                ui.end_row();
            });
